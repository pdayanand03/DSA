...
You will now see that the beginning of the array is in the correct order (similar to how after the first iteration of bubble sort,
...
function selectionSort(array, comparator=null) {
  const swap = (arr, index1,index2) =>[arr[index1],arr[index2]]=[arr[index2],arr[index1]]
  let lowIndex;
  for(let i=0; i<array.length-1; i++){
    lowIndex=i;
    for(let j=i; j<array.length; j++){
      if(typeof comparator !== 'function' && array[j]<array[lowIndex]){
        lowIndex=j;
      }
      if(typeof comparator === 'function' && comparator(array[lowIndex], array[j])>0){
        lowIndex=j
      }
    }
    if(lowIndex!== i){
