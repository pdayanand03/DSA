/*
Insertion Sort
Implement insertionSort  . Given an array, both algorithms will sort the values in the array. The functions take 2 parameters: 
an array and an optional comparator function. The comparator function is a callback that will take two values from the array to be 
compared. The function returns a negative value if the first value is less than the second, a positive value if the first value is 
greater than the second, and 0 if both values are equal. The default comparator you provide should assume that the two parameters 
...
function insertionSort(array, comparator=null) {
  const swap = (arr,ind1,ind2) => ([arr[ind1],arr[ind2]] = [arr[ind2],arr[ind1]])
  for(let i=1; i<array.length; i++){
    let j=i;
    while(j>=1){
      if(typeof comparator !== 'function' && array[j-1] > array[j]){
        //swap
        swap(array, j, j-1);
      }else if(typeof comparator === 'function' && comparator(array[j-1], array[j])>0){
        //swap`
        swap(array, j, j-1);
      }else{
        break;
