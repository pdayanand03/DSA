...
Note that a subarray must consist of consecutive elements from the original array. In the first example below, [100, 200, 300] is a subarray
...
function maxSubarraySum(arr,index) {
    ...
    let tempSum=0, maxSum,i;
    for(i=0; i<index; i++){
        tempSum = tempSum + arr[i];
    }
    maxSum=tempSum;
    for(let j=i; j<arr.length; j++){
        tempSum = tempSum + arr[j] - arr[j-index];
        if(tempSum > maxSum){
            maxSum = tempSum
        }
    }
