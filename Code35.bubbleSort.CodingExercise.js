/*
Bubble Sort
ATTEMPT THIS IS YOU ARE UP FOR IT! Implement a function called bubbleSort. Given an array, bubbleSort will sort the values in 
the array. The function takes 2 parameters: an array and an optional comparator function.
...
The function returns a negative value if the first value is less than the second, a positive value if the first value is greater 
...
function bubbleSort(array, comparator=null) {
  ...
  for(let i=array.length-1; i>0; i--){
    for(let j=0; j<i; j++){
      if(typeof comparator !== "function" && array[j] > array[j+1]){
        swap(array,j,j+1);
      }
      if(typeof comparator === "function" && comparator(array[j],array[j+1])>0){
        swap(array,j,j+1);
      }
    }
  }
